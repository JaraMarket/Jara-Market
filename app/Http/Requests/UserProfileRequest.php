...
class UserProfileRequest extends FormRequest
{
    ...
    {
        $authUser = $this->user(); // logged-in user
        $userId = $this->route('id') ?? $authUser->id; 
        // adjust if your route uses a different param name for user ID

        $phoneRule = ['nullable', 'string'];

        // Apply unique rule ONLY if not admin AND not the owner
        if ($authUser->role !== UserPermissionsEnum::ADMIN() && $authUser->id !== (int) $userId) {
